...
export const BrowserClick = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    return {
      description:
        "Click on the page. Two ways to target, pick whichever fits: (1) elementId from browser_getdom — best when the page has meaningful DOM; or (2) x,y coordinates read from a browser_screenshot — best for canvas/visual pages, where the screenshot's pixels map 1:1 to these coordinates. Provide elementId OR both x and y. Coordinates are viewport-relative, so take a fresh browser_screenshot right before using them (don't reuse stale coordinates after scrolling/navigation). After a click that may navigate, re-run browser_getdom or browser_screenshot to refresh. A red crosshair marker is painted at the click location and persists until the next browser_screenshot — so the next screenshot shows where the click landed, letting you verify coordinate accuracy and self-diagnose misfired clicks. Offscreen elements CANNOT be clicked — if elementCenter reports the element is offscreen, the click returns an 'Element ... is offscreen' error and you MUST use browser_scroll to bring it into view (page-relative deltas: dy=600 to scroll down one viewport, dy=-600 to scroll up), re-screenshot, then retry. Check the y coordinate in browser_getdom output: if y < 0 or y > viewport height, the element is offscreen and you need to scroll first. If a click by (x,y) coordinates seems to miss (the marker in the next screenshot is offset from your target), recalibrate from that screenshot; if the page didn't respond at all, scroll to ensure the target is in viewport and retry.",
